  ...
  useEffect(() => {
    ...
    mapRef.current.on("draw.update", (e: { features: GeoJSON.Feature[] }) => {
      e.features.forEach((feature) => {
        const fieldId = feature.id as string;
        const updatedCoordinates = (feature.geometry as GeoJSON.Polygon)
          .coordinates;
        ...
        if (mapRef.current?.getSource(fieldId)) {
          (mapRef.current.getSource(fieldId) as mapboxgl.GeoJSONSource).setData(
            feature
          );
        }
        ...
        if (mapRef.current?.getSource(labelLayerId)) {
          (
            mapRef.current.getSource(labelLayerId) as mapboxgl.GeoJSONSource
          ).setData(feature);
        }
        ...
        if (mapRef.current?.getSource(borderLayerId)) {
          (
            mapRef.current.getSource(borderLayerId) as mapboxgl.GeoJSONSource
          ).setData(feature);
        }
        ...
        if (mapRef.current?.getSource(iconLayerId)) {
          (
            mapRef.current.getSource(iconLayerId) as mapboxgl.GeoJSONSource
          ).setData(feature);
        }
      });
    });
    ...
    geolocate.on("error", (e) => {
      ...
      toast.error(
        `Error getting location: ${e.message}. Note: Geolocation requires a secure connection (HTTPS) or localhost.`
      );
    });
